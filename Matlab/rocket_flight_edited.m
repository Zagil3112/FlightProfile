...
fprintf('\n Drag loss				               = %10g Km/s',vlossdrag1st(end))*1.e-3
fprintf('\n Gravity loss				            = %10g Km/s',vlossgrav1st(end))*1.e-3
fprintf('\n')
fprintf('\n --------------SECOND FLIGHT PHASE----------------------')
fprintf('\n                      Ballistic                         ')
...
fprintf('\n Drag loss				               = %10g Km/s',(vlossdrag2nd(end))-vlossdrag2nd(1))*1.e-3
fprintf('\n Gravity loss				            = %10g Km/s',(vlossgrav2nd(end))-vlossgrav2nd(1))*1.e-3
fprintf('\n')
fprintf('\n --------------THIRD FLIGHT PHASE-----------------------')
fprintf('\n                     Powered                            ')
...
fprintf('\n Propellant mass burned              = %10g Kg',mprop_toma)
fprintf('\n Tomahawk ignition time              = %10g s',tburn_toma)
fprintf('\n Mission elapsed time                = %10g s',t3(end))
fprintf('\n Drag loss				               = %10g Km/s',(vlossdrag3rd(end))-vlossdrag3rd(1))*1.e-3
fprintf('\n Gravity loss				            = %10g Km/s',(vlossgrav3rd(end))-vlossgrav3rd(1))*1.e-3
fprintf('\n')
fprintf('\n --------------FOURTH FLIGHT PHASE----------------------')
fprintf('\n                     Ballistic                          ')
...
fprintf('\n Drag loss				               = %10g Km/s',(vlossdrag4th(end))-vlossdrag4th(1))*1.e-3
fprintf('\n Gravity loss				            = %10g Km/s',(vlossgrav4th(end))-vlossgrav4th(1))*1.e-3
fprintf('\n')
fprintf('\n --------------FIFTH FLIGHT PHASE-----------------------')
fprintf('\n                     Powered                            ')
...
fprintf('\n Drag loss				               = %10g Km/s',(vlossdrag5th(end))-vlossdrag5th(1))*1.e-3
fprintf('\n Gravity loss				            = %10g Km/s',(vlossgrav5th(end))-vlossgrav5th(1))*1.e-3
fprintf('\n')
fprintf('\n --------------SIXTH FLIGHT PHASE-----------------------')
fprintf('\n                     Ballistic                          ')
...
fprintf('\n Drag loss				               = %10g Km/s',(vlossdrag6th(end))-vlossdrag6th(1))*1.e-3
fprintf('\n Gravity loss				            = %10g Km/s',(vlossgrav6th(end))-vlossgrav6th(1))*1.e-3
fprintf('\n')
...
fprintf('\n Total Drag loss (only ascent)             = %10g Km/s',vlossdrag(end))
fprintf('\n Total Gravity loss (only ascent)          = %10g Km/s',vlossgrav(end))
fprintf('\n\n ---------------------------------------------------\n')
...
tiempo_over100 = t(over100_0:over100_f);
delta_over100= delta(over100_0:over100_f)*deg;
lamda_over100=lamda(over100_0:over100_f)*deg;
phi_over100=phi(over100_0:over100_f)*deg;
v_over100=v(over100_0:over100_f);
Azimut_over100=A(over100_0:over100_f)*deg;
radio_over100 = r(over100_0:over100_f)*1.e+3;
h_over100 = h(over100_0:over100_f);

%x,y,z coordenadas trayectoria total

x_t = (r*1.e+3).*cos(delta*deg).*cos(lamda*deg);
y_t = (r*1.e+3).*cos(delta*deg).*sin(lamda*deg);
z_t = (r*1.e+3).*sin(lamda*deg);

%coordenadas en metros 
x = radio_over100.*cos(delta_over100).*cos(lamda_over100); % delta = latitud , lamda = longitud 
y = radio_over100.*cos(delta_over100).*sin(lamda_over100);
z = radio_over100.*sin(lamda_over100); 

% coordenadas en km 
x_2 = radio_over100.*cos(delta_over100).*cos(lamda_over100)*1.e-3; 
y_2 = radio_over100.*cos(delta_over100).*sin(lamda_over100)*1.e-3;
z_2 = radio_over100.*sin(delta_over100)*1.e-3; 

%Velocidades VxNorth,VyEast, VzDown
North = v_over100.*cos(Azimut_over100).*cos(phi_over100);
East = v_over100.*cos(phi_over100).*sin(Azimut_over100);
Down= -1*v_over100.*sin(phi_over100);

%Vx,Vy,Vz
[Vx,Vy,Vz]=ned2ecefv(North,East,Down,delta_over100,lamda_over100,'radians');

Tabelle = table(h_100,tiempo_over100,delta_over100,lamda_over100, phi_over100, ...
    v_over100,Azimut_over100,h_over100,radio_over100,x_2,y_2,z_2,North,East,Down,Vx,Vy,Vz);




Tabelle.Properties.VariableNames ={'Posicion','Tiempo_s','Latitud_rad','Longitud_rad','Angulo_horizonte_rad','Velocidad_km_s'...
                                    'Azimut_rad','Altura_km','Radio_m','X','Y','Z','North_k_s','East_k_s','Down_k_s','Vx','Vy','Vz'};
%Tabelle.Propierties.VariableUnits= {' ','seg','rad','rad','rad','km/s','rad','km','m','km','km','km'};
writetable(Tabelle,'Perfil_de_vuelo(datos).xlsx');

Tabelle2 = table(tiempo_over100,x_2,y_2,z_2,Vx,Vy,Vz);
Tabelle2.Properties.VariableNames = {'Tiempo','X','Y','Z','Vx','Vy','Vz'};
writetable(Tabelle2,'Vector_de_Estado2.xlsx');



opt = input('Plot x,y,z?\n1.Yes\n2.No\nWaiting for input...');
