function xprime = powered_tomahawk(t,x);
...
D = 1/2*rho*v^2*A_toma*Cd;

xprime = [Th_toma/m-D/m-gc*sin(phi)+gdelta*cos(phi)*cos(A)-omega^2*r*cos(delta)*(cos(phi)*cos(A)*sin(delta)-sin(phi)*cos(delta)); ...
